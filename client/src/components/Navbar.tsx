  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-50 transition-colors duration-300 ${scrolled ? "bg-dark/95 border-b border-white/5" : "bg-transparent"}`}>
      <div className="container flex items-center justify-between h-16">
        ...
        <button onClick={() => setOpen(!open)} className="md:hidden text-white/70" aria-label="Menu">
          {open ? <X size={22} /> : <Menu size={22} />}
        </button>
      </div>

      {open && (
        <div className="md:hidden bg-dark border-t border-white/5 px-4 py-4 space-y-3">
          {links.map((l) => (
