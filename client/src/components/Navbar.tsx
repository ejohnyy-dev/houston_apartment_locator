...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 inset-x-0 z-50 transition-colors duration-300 ${scrolled ? "bg-dark/95 border-b border-white/5" : "bg-transparent"}`}
    >
      <div className="container flex items-center justify-between h-16 md:h-18">
        ...
        <div className="hidden lg:flex items-center gap-7">
          ...
          <a
            href="/#contact"
            className="px-5 py-2 bg-gold text-dark text-sm font-semibold rounded hover:opacity-90 transition-opacity"
          >
            Get Started
          </a>
        </div>
        ...
        <div className="lg:hidden bg-dark border-t border-white/5 px-4 py-4 space-y-3">
          {links.map(l => (
            <a
              ...
              className="block text-white/60 hover:text-white text-sm font-medium"
            >
              {l.label}
            </a>
          ))}
          ...
          <a
            href="/#contact"
            onClick={() => setOpen(false)}
            className="block text-center py-2.5 bg-gold text-dark text-sm font-semibold rounded"
          >
            Get Started
          </a>
        </div>
