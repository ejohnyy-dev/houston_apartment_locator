...
// Generate 45 bars with right-skewed distribution (peak ~30% of range, long tail to right)
function generateHistogramBars(count: number): number[] {
  const bars: number[] = []
  const peakIdx = Math.floor(count * 0.3) // peak at ~30%

  for (let i = 0; i < count; i++) {
    // Rising side: steeper curve on the left
    let normalized: number
    if (i <= peakIdx) {
      normalized = i / peakIdx
      // Ease-out cubic for rising side
      normalized = 1 - Math.pow(1 - normalized, 3)
    } else {
      normalized = (i - peakIdx) / (count - 1 - peakIdx)
      // Ease-out quad for falling side (slower decay)
...
export default function BudgetRangeSelector({
  ...
  // Click on histogram to move nearest handle
  const handleHistogramClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Determine which handle to move: closest wins
  ...
  const countText = availableCount.toLocaleString()
      ...
      <div className="relative mb-6 px-1">
        ...
        <div className="relative h-1.5 mt-2 bg-slate-200 rounded-full mx-0">
          ...
          >
            <div
              className={`
                w-5 h-5 rounded-full bg-white border-2 border-slate-300
                shadow-md flex items-center justify-center cursor-grab
                transition-transform duration-100
                ${dragging === 'min' ? 'scale-110 cursor-grabbing' : ''}
                ${hoveredHandle === 'min' ? 'scale-105' : ''}
              `}
              style={{
                borderColor: dragging === 'min' || hoveredHandle === 'min' ? '#3b82f6' : '#cbd5e1',
              }}
            >
          ...
          >
            ...
            <div
              className="
                ...
                bg-white text-slate-900 text-xs font-semibold px-2 py-1 
                rounded-lg shadow-md border border-slate-200
            ...
            <div
              className={`
                ...
                ${hoveredHandle === 'max' ? 'scale-105' : ''}
              `}
              style={{
                borderColor: dragging === 'max' || hoveredHandle === 'max' ? '#3b82f6' : '#cbd5e1',
              }}
            >
              <div className="w-1.5 h-1.5 rounded-full bg-slate-400" />
            </div>
      ...
      <div className="flex items-end gap-2 mb-4">
        <div className="flex-1">
          ...
          <input
            ...
            placeholder="No min"
            className="w-full px-2 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg
                       placeholder-slate-400 outline-none transition-colors duration-150
        ...
        <div className="flex-1">
          ...
          <div className="relative">
            <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-slate-900 font-medium">
              $
            </span>
            <input
              type="text"
              ...
              className="w-full pl-5 pr-2 py-2 text-xs text-slate-900 bg-white border border-slate-300 rounded-lg
