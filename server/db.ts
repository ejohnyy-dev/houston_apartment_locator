...
function initializeSchema() {
  ...
  // Leads table (txaptfinder.com version - enhanced for nurturing)
  database.exec(`
    CREATE TABLE IF NOT EXISTS leads (
      ...
      notes TEXT,
      nurtureStage TEXT DEFAULT 'none',
...
export async function createLead(data: { 
  ...
  const database = getDb();
  const stmt = database.prepare(
    'INSERT INTO leads (name, email, phone, moveTimeline) VALUES (?, ?, ?, ?)'
  );
  const info = stmt.run(data.name, data.email, data.phone, data.moveTimeline || null);
  return info;
}
